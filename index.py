...
def retrieve_files():
	...
	with open(os.path.join(my_dir, 'docnames.txt'), 'r') as fh:
...
def phrase_query(query_lemmas):
    ...
    print("\n\nPhrase Quering:\n")
    print ("No relevant document!" if (len(phrase_query_docs) < 1) else "As a fixed phrase found in ", len(phrase_query_docs), " documents in total:", ",".join(phrase_query_docs))
    print()
