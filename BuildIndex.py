...
if (__name__ == "__main__") :
	...
	for file in os.listdir(line_args.input_dir):
		...
		with open(line_args.input_dir + file, "r") as fh:
			...
			# Our inverted index would map words to document names but, we also want to support phrase queries: queries for not only words, 
			...
			for line in fh:
				for word, pos in pos_tag(wp_tokenizer.tokenize(line.lower().strip())):
					if(
						...
						excluded_words += 1
						continue
					# If current term's appearance is verb related then 
					# the POS lemmatizer should be verb ('v'), otherwise ('n')
					pos = 'v' if (pos.startswith('VB')) else 'n' 
